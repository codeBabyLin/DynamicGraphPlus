...
trait Clauses extends Parser
  ...
  def MatchHistory: Rule1[ast.MatchHistory] = rule("MATCHHISTORY") {
    group((
      keyword("OPTIONAL MATCHHIS") ~ push(true)
  ...
  def CreateV: Rule1[ast.CreateV] = rule("CREATEWithV") {
    group(keyword("CREATEV") ~~ Pattern ~~ optional(At)) ~~>> (ast.CreateV(_,_))
  }
