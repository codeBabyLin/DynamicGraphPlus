...
case class MatchHistory(
  ...
  override def name = "MATCHHISTORY"
...
/*
  def copy(optional: Boolean = this.optional,pattern: Pattern = this.pattern,
           hints: Seq[UsingHint] = this.hints,
           where: Option[Where] = this.where,at:Option[At] = this.at)(position: InputPosition = this.position): Match ={
    new Match(optional,pattern,hints,where,at)(position)
  }
*/
...
case class CreateV(pattern: Pattern,at:Option[At])(val position: InputPosition) extends UpdateClause with SingleRelTypeCheck {
  override def name = "CREATEV"
