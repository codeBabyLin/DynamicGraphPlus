...
import org.neo4j.cypher.internal.v3_5.logical.plans.{GraphVersionSelect, LogicalPlan}
...
case class PlanSingleQuery(planPart: PartPlanner = planPart,
  ...
  override def apply(in: PlannerQuery, context: LogicalPlanningContext, idGen: IdGen): (LogicalPlan, LogicalPlanningContext) = {
    ...
    var newPlan = finalPlan
    if(in.queryGraph.graphVersion.nonEmpty){
      newPlan = GraphVersionSelect(in.queryGraph.graphVersion.head.expr,finalPlan)(idGen)
      val pQ = finalContext.planningAttributes.solveds.get(finalPlan.id)
      finalContext.planningAttributes.solveds.set(newPlan.id,pQ)
    }
    //(verifyBestPlan(finalPlan, in, finalContext), finalContext)
    //verifyBestPlan(finalPlan, in, finalContext)
   // val pQ = finalContext.planningAttributes.solveds.get(finalPlan.id)
   // finalContext.planningAttributes.solveds.set(newPlan.id,pQ)
   // (newPlan, finalContext)
    (verifyBestPlan(newPlan, in, finalContext), finalContext)
  }
}
