...
sealed class TransactionBoundQueryContext(val transactionalContext: TransactionalContextWrapper,
  ...
  override def createNode(labels: Array[Int]): NodeValue = ValueUtils.fromNodeProxy(entityAccessor.newNodeProxy({
    if(this.isV) writes().nodeCreateWithLabelsWithVersion(labels,this.version)
    else writes().nodeCreateWithLabels(labels)
  }))
  ...
  class NodeOperations extends BaseOperations[NodeValue] {
    ...
    override def setProperty(id: Long, propertyKeyId: Int, value: Value): Unit = {
      try {
        if(TransactionBoundQueryContext.this.isV){
          writes().nodeSetPropertyWithVersion(id, propertyKeyId, value,TransactionBoundQueryContext.this.version)
        }
        else writes().nodeSetProperty(id, propertyKeyId, value)
      } catch {
        case _: api.exceptions.EntityNotFoundException => //ignore
      }
    }
      ...
      new CursorIterator[NodeValue] {
        override protected def fetchNext(): NodeValue = {
          if (nodeCursor.next()) {
            ...
            if(TransactionBoundQueryContext.this.isV){
              val nodeV: Long = DGVersion.getStartVersion(nodeCursor.nodeVersion())
              val v:Long = TransactionBoundQueryContext.this.version
              if(nodeV == v) fromNodeProxy(entityAccessor.newNodeProxy(nodeCursor.nodeReference()))
              else this.fetchNext()
            }
            else fromNodeProxy(entityAccessor.newNodeProxy(nodeCursor.nodeReference()))
          } else null
        }
